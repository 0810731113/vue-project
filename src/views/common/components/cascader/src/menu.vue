<script>
  ...
  const copyArray = (arr, props) => {
    ...
    const result = [];
    const configurableProps = ['__IS__FLAT__OPTIONS', 'label', 'value', 'parentId', 'GrandPId', 'disabled', 'level'];
...
//                'mw-cascader-item': itemIndex === 1,
        ...
        const addLabel = menuIndex === 0 ? '新增分类' : menuIndex === 1 ? '新增子分类' : '新增二级子分类';
        const addMenuEvent = {
          on: {}
        };

        addMenuEvent.on.click = () => {
          this.$emit('addNewMenu', menu);
          this.$emit('closeInside');
        };
