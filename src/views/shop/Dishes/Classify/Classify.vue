  ...
  <el-form class="classify-form mw-form" :rules="rules" ref="form" :model="form" label-width="120px">
    ...
    <div v-if="!isSetMeal && checkIsShop()">
      ...
      <el-form-item prop="menuClsMuldept" v-show="(form.menuClsKind == MENUCLS_KIND[0].label) && (form.isPrn == MENU_PRINT_TYPE[2].id)">
        <el-select v-model="form.printPlanName">
          <el-option
        ...
        <el-table :data="printPlanList" border style="width: 100%;" max-height="300">
...
import { AttributionModal } from '../components';
...
export default {
  ...
  data () {
    return {
      ...
      attributionType: '',
      menuClsError: '',
      // 新增菜品页面中只有新增入口 isDish标识在新增菜品中调用
      isEdit: this.isDish ? false : (!!this.$route.params.id || !!this.editId)
    };
  },
  computed: mapState({
    ...
    expclsList (state) {
      return state.dishes.classify.expclsList;
    },
    revenueTypeList (state) {
      return state.dishes.classify.revenueTypeList;
    },
  ...
  watch: {
    async 'form.printPlanName' () {
      if (this.form.printPlanName) {
        await this.fetchPrintPlanList({ printPlanName: this.form.printPlanName });
        this.form.menuClsMuldept = this.printPlanList;
      }
    }
  },
  async created () {
  ...
  methods: {
    ...
    async onSubmit () {
      ...
      if (this.isGuide && !errorStatus) {
        if (this.onClickSubmit) this.onClickSubmit();

        this.$emit('onSubmit', [...this.form.level, this.form.menuClsId.toString()]);
        return;
      }
      if (!errorStatus) {
        this.$router.back();
