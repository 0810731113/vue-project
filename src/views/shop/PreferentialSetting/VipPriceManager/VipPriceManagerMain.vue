  ...
  <div class="vip-price">
    <el-form :inline="true" :model="searchForm" class="search-bar">
      ...
      <el-form-item label="菜品分类:">
        <el-cascader expand-trigger="hover" :options="classifyList" :props="defaultProps" v-model="searchForm.fsMenuClsId">
        </el-cascader>
      </el-form-item>
      <el-form-item label="会员卡:">
        <el-select placeholder="请选择" v-model="searchForm.csId" @change="getLevelList">
          <el-option v-for="item in cardList" :key="item.csId" :label="item.cardName" :value="item.csId">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="价格范围:">
        <el-select placeholder="会员等级" v-model="searchForm.LevelValue">
          ...
          <el-option v-for="item in cardLevel" :key="item.level" :label="item.title" :value="item.level">
          </el-option>
        </el-select>
        <input-price
    ...
    <el-table :data="viplist.list" ref="tablelist" border @selection-change="handleSelectionChange" @row-click="handleRowClick">
      ...
      <el-table-column v-for="item in viplist.vipColumns" :key="item.level" :label="item.vipPrice" align="center">
        <template slot-scope="scope">
          {{ getProperty(item.level,scope.row, viplist.vipColumns) }}
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <mw-button class="btn-manage" content="设置" :disabled="isMaintain" @click.stop="handleView(scope.$index, scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    <pagination
    ...
    <popup
      ...
      <div slot="content">
        <el-form label-width="90px" :model="collectForm" class="collect-form" ref="collectForm">
          <el-form-item label="菜品编号:" prop="fsItemId">
            {{ collectForm.fsItemId }}
          </el-form-item>
          <el-form-item label="菜品名称:" prop="fsItemName">
            {{ collectForm.fsItemName }}
          </el-form-item>
          <el-form-item label="原价:" prop="salePrice">
            {{ `${collectForm.salePrice}/${collectForm.orderUint}` }}
          </el-form-item>
          <p class="warn-text">说明：会员价“是否”赠送功能为新功能。安卓为V3.0.1（windows V2.8.1）及以上版本设置后生效；老版本设置“赠送”后，执行菜品原价。</p>
    ...
    <popup
      ...
      <div slot="content">
        ...
        <el-row class="row-setting">
          <el-col :span="8">
            <el-select placeholder="会员等级" v-model="batchForm.levelValue" style="display: inline-block">
              <el-option v-for="item in realCardsLevel" :key="item.level" :label="item.title" :value="item.level">
              </el-option>
            </el-select>
          </el-col>
          <el-col :span="4" style="line-height: 35px;text-align: right; margin-right: 10px">设置为原价</el-col>
          <el-col :span="6">
            <el-input-number style="width: 120px;" v-model="batchForm.ratio" :min="1" :max="100" :precision="0" controls-position="right"/>
...
//      },
    ...
    methods: {
      ...
      switchBlackCardChange(event,row){ //黑卡设置
        console.log(row)
        row['switch'] = event
        if(event){
          row['cpPrice'] = 0
      ...
      onBlurBlackCardPrice(event, row){ //黑卡设置
        const { value } = event.target;
      ...
      async handleView (index, row) {
        ...
        await this.getVipItemDetail({ itemCd, orderUnitCd: orderUintCd,csId,bindType:this.getBindType(csId) });
        this.collectForm = {
          ...this.collectForm,
          fsItemId: itemId,
          fsItemName: itemName,
...
//          }
        ...
        await this.updateOneVIPBargain({ itemCd, orderUintCd, requestData, cpRequestData,csId, bindType: this.getBindType(csId)});
        await this.getVipList(this.getFormParams());
      ...
      async submitBatchSet () {
        ...
        this.multipleSelection.map(item => {
          requestData.push({ itemCd: item.itemCd, orderUintCd: item.orderUintCd, srcPrice: item.salePrice });
        });
        const { levelValue, ratio } = this.batchForm;
        const { csId } = this.searchForm
        await this.bashSetVIPBargains({ csId, bindType:this.getBindType(csId), level: levelValue, rate: ratio/100, requestData });
        await this.getVipList(this.getFormParams());
